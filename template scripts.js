...
// Stats counter animation (usado si hay números reales)
const animateStats = () => {
    const stats = document.querySelectorAll('.stat-number');
    stats.forEach(stat => {
        // Si el contenido no es numérico (por ejemplo “”), evitamos animar
        const numeric = stat.textContent.replace(/[^\d]/g, '');
        if (!numeric) return;

        const target = parseInt(numeric);
        let count = 0;
...
// Random cyber text effects (adaptado a español)
const cyberTexts = [
    'CONECTANDO CON ÑUBLE...',
    'DATOS TURÍSTICOS ACTUALIZADOS',
    'GAMIFICACIÓN ACTIVA',
    'MAPAS Y RUTAS INTERACTIVAS',
...
// Contact form submission (simulada)
